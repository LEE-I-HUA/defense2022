...
def get_urls(opinion_code, last_link):    
    ...
    while turn:
        ...
        try:
            res = requests.get(url, headers=headers)#發送HTTP Request
            for key in res.json()["items"]:
                # 獲取文章url
                information = key["html"]
                newsHerf = re.search("https.*/", information).group(0)#尋找包含https的url
...
def opinion_insert_database(**context):
    # 獲取opinion_get_content_task的輸出結果
...
def token_replace():
	collection = connect_db()
    # 找出'tokens'相關的資料，並取出'content'的部分
	...
	if len(origin_data)>0:
		...
		print('需要更新斷詞的文章數：0')
        #json_util.dumps()轉換成json格式，json.loads()讀取json數據
	return json.loads(json_util.dumps(new_data))


# 斷詞
def tokenize(**context):
    # 獲取token_replace_task的輸出結果
	...
	if len(total_new_data) == 0:
		...
		contentDF = pd.json_normalize(total_new_data).rename(columns={'_id.$oid':'_id'})#轉成dataframe格式
        ...
        # 取出contentDF['index_','content']的值作為ids,texts
...
def dtm_pre_process():
    ...
    #                     {'_id':0,'tokens':1}))
    # 取出所有'tokens'和'sentence'的資料，進行排序，並新增['doc_id']
    ...
    # 對['sentence']進行explode後，每一列給予sen_id
	...
	print('製作DTM前處理')
	start = datetime.now()
    # 對['tokens']進行explode後，每一列給予sen_id和token

...
# In[]
# 字典
origin_key_dict_pd = pd.read_csv('./new_data/entityDict.csv')
...
def node_recation(Unit, data, type, total_nodes_num, threshold):
    ...
    merged_df['artDate_Url'] = merged_df.apply(lambda row: html.A(html.P(row['artDate']), href=row['artUrl']), axis=1)
...
def edge_recation(Unit, data, type, total_nodes_num, threshold):
    ...
    #將artDate加入超連結功能，測試中        
    merged_df2['artDate_Url'] = merged_df2.apply(lambda row: html.A(html.P(row['artDate']), href=row['artUrl']), axis=1)

...
sliderInput("K",  "設定網路節點數量",  2, 10, 5, 1)
...
# 可以匯出目前參數網址
AppUrl = "https://ba.cm.nsysu.edu.tw:4950/m104020035/Ego.Rmd"
...
SX = reactive({
  #input = list(Cor=0, Unit=1,Z="Carnegie_Mellon_University",filter_= "不篩選", VN=list(nodes=162), K=7, Q=0.5) # APP外測試，debug用
  VN = input$VN
...
renderVisNetwork({
  ...
  if(input$filter_ !="不篩選"){
    si0 = si0[(E$class[si0] %in% input$filter_ | E$entity[si0] == input$Z)]
  }
  si0 = si0[1:input$K]
  ...
  for(i in c(1:input$K)){
      if(input$Unit == 2){
        v = CR$Docu[which(E$entity == E$entity[si0[i]]),]}else{ 
          v = CR$Sent[which(E$entity == E$entity[si0[i]]),]
          }
      ...
      if(input$filter_ !="不篩選"){
        si = si[(E$class[si] %in% input$filter_ | E$entity[si] == E$entity[si0[i]])]
      }
      si = si[1:input$K]
      siFactorial<- append(siFactorial, si) #取得所有的node於 siFactorial list
  }
  
  # 去除掉重複值  
  siFactorial <- unique(siFactorial)
  
  # 轉換型態為整數
  sf <- as.integer(siFactorial) 
  ...
  colnames(x) = rownames(x) = sf
  x[lower.tri(x,T)] = 0
  x = subset(melt(x), value > 0) #取兩兩 keyword 間正相關的資料

---
title: "國防 太空文集2017~2022 即時單中心網路分析"
output: 
...
class_list=list(
     ...
     "軍事"=c("軍隊","武器","基地"),
     "抽象事物"=c("計畫","戰術","媒體","金融/法律/合約","新增關鍵字")
     )
...
textInput("pattern","輸入自訂關鍵字","OQ Technology")
...
pickerInput(
  inputId="filter_", label = "網路篩選遮罩", choices = c("不篩選",unique(E$class)), multiple=T,
  selected = "不篩選")
h6("針對網路圖的節點可以進行篩選")
...
actionBttn("dump","立即計算關鍵字網路",style="unite",color="danger",size="s") # 匯出目前參數按鈕

# 可以匯出目前參數網址
# AppUrl = "https://ba.cm.nsysu.edu.tw:4950/mirdc/defense2022/Ego.Rmd"
...
filter_node_result = eventReactive(input$dump, {
  #input = list(pattern = "cubsat",Cor=2, Unit=1,Z="ABMS",VN=list(nodes=474), K=10, Q=0.5) #debug用
  #input = list(pattern = "Hydrosat",Cor=2, Unit=2,Z="ABMS",VN=list(nodes=474), K=20, Q=0.65) #debug用nodes=474 edges="474-14"
  ...
  #把新的字放進字典裡面
  e = rbind(e,list(input$pattern,"keyword","keyword"," ","一般術語"," ","keyword",as.numeric(count),as.logical(TRUE),""))
  
  #計算新字在sent有無出現
  pattern_stm = str_detect(S$sen, regex(input$pattern, ignore_case=T) )
  
  #把計算結果加入x_sent中
  x_sent = cbind(x_sent,pattern_stm)
  #計算新字在docu有無出現
  #pattern_xDocu = str_detect(X$artContent, regex(input$pattern, ignore_case=T) )
  pattern_xDocu = sapply(split(data.frame(pattern_stm), S$doc_id), colSums) > 0 
  #把計算結果加入x_docu中
  ...
  #Sys.time() - start_time #15.46752 secs
  
  ####CO
  #start_time <- Sys.time()
  co$Sent = t(x_sent) %*% x_sent
  co$Docu = t(x_docu) %*% x_docu
  #Sys.time() - start_time # 26.62209 mins
...
SX = reactive({
  ...
  #result = list( e=e ,x_sent=x_sent,cr=cr,co=co,x_docu=x_docu)
...
renderVisNetwork({
  result=filter_node_result()
  #result = list( e=e ,x_sent=x_sent,cr=cr,co=co) #debug用
...
renderDataTable({
  if(class(SX()) == 'list') {
    ...
    for(j in 1:length(k)) df$z = str_replace_all(
      #這裡
      df$z, regex(paste0('(',result$e$alias[k[j]],')'), ignore_case = result$e$ignore[k[j]]),
...
output$myTable <- renderDataTable({
  if(!is.null(SX())) {
    result=filter_node_result()
    k = SX()$k; df = SX()$df[,c('artDate', 'nid','sentence', 'artUrl')]#, 'artUrl'
    df$z = df$sentence
    ...
    for(i in 1:length(k)) {
      ...
      df$z, regex(paste0('(',result$e$alias[k[i]],')'), ignore_case = result$e$ignore[k[i]]),
